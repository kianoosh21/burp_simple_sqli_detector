...
class BurpExtender(IBurpExtender, ITab, IContextMenuFactory):
    ...
    def _build_ui(self):
        ...
        self.threads_field = JTextField("1", 4)
        self.timeout_field = JTextField("30", 4)

        self.debug_cb = JCheckBox("--debug", False)
        self.force_json_cb = JCheckBox("--json", False)

        self.mode_sr = JRadioButton("-sr / --singleRecursive", True)
        self.mode_faster = JRadioButton("-faster", False)
        self.mode_fastest = JRadioButton("-fastest", False)
        bg = ButtonGroup()
        bg.add(self.mode_sr)
        bg.add(self.mode_faster)
        bg.add(self.mode_fastest)

        self.scope_default = JRadioButton("Default scope (params)", False)
        self.scope_cookie = JRadioButton("-c / --cookie-only", False)
        self.scope_full = JRadioButton("-f / --full", True)
        sbg = ButtonGroup()
        sbg.add(self.scope_default)
        sbg.add(self.scope_cookie)
        sbg.add(self.scope_full)

        self.btn_stop_now = JButton("Stop NOW")
        self.btn_clear_jobs = JButton("Clear jobs")
        self.btn_clear_results = JButton("Clear results")

        self.btn_stop_now.addActionListener(StopNowListener(self))
        ...
        mi_cmp2 = JMenuItem("Send shown request to Comparer")
    ...
    def stop_now(self):
        # clear queued jobs immediately
        self._clear_queue_only()

        with self.current_lock:
            running = (self.current_job_id is not None)

        if running:
            self.stop_event.set()
            self._log_line("[*] Stop NOW requested (best-effort; stops as soon as current HTTP call returns)")
        else:
            self._log_line("[*] Stop NOW: queue cleared (no running job)")
    ...
    def _find_row_by_jobid(self, jobid):
        # called inside EDT via invokeLater in _ui_set_cell
        key = str(jobid)
    ...
    def _log_hit_param_red(self, prefix, param_name, suffix):
        def do():
            try:
                doc = self.results_pane.getStyledDocument()
                doc.insertString(doc.getLength(), prefix, None)

                style = self.results_pane.addStyle("hit_red", None)
                StyleConstants.setForeground(style, Color.RED)
                doc.insertString(doc.getLength(), param_name, style)

                doc.insertString(doc.getLength(), suffix, None)
                self.results_pane.setCaretPosition(doc.getLength())
            except Exception:
                pass
        SwingUtilities.invokeLater(do)
    ...
    def _get_debug(self):
        return True if self.debug_cb.isSelected() else False
...
class StopNowListener(ActionListener):
    def __init__(self, ext):
        self.ext = ext
    def actionPerformed(self, event):
        self.ext.stop_now()
...
class Worker(Runnable):
    ...
    def run(self):
        # CRITICAL: never let the worker die. Any exception must be caught and loop must continue.
        while True:
            try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        self.ext._log_line("[%s] ERROR: Stopped by user" % job.id)
                    except Exception:
                        pass
    ...
    def _run_job(self, job):
        ...
        # IMPORTANT: --json means JSON-only mode (POST JSON body only).
        # It must NOT run full/cookies/headers and must NOT use faster/fastest.
        if force_json:
            mode = "single"
            cookie_only = False
            full_mode = False
        ...
        if hits > 0:
            ...
            # Auto-fill JobID into the search field and show initiating request
            try:
